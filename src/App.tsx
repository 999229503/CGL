...
function App() {
  ...
  const salvarAlteracaoImediata = (dados: CloudDataComHistorico, marcarSalvamentoDireto = true): Promise<void> => {
    ...
    const snapshot = JSON.parse(JSON.stringify(dadosComHistorico)) as CloudDataComHistorico;

    // O cache local é atualizado imediatamente. Assim a tela não fica
    // esperando o Supabase para considerar a alteração salva.
    try {
      localStorage.setItem(CHAVE_DADOS, JSON.stringify(snapshot));
      const listas: Record<string, unknown> = snapshot as unknown as Record<string, unknown>;
