...
import { cloudConfigured, getSession, getCloudDataUpdatedAt, loadCloudData, login, logout, saveCloudData, type AuthSession, type CloudData } from "./cloud";
...
function App() {
  ...
  useEffect(() => {
    ...
    const sincronizarDeOutroDispositivo = async () => {
      ...
      // Em segundo plano não há necessidade de consultar o servidor.
      // O navegador chama uma sincronização assim que o CGL voltar a ficar visível.
      ...
      try {
        ...
        // Primeiro baixa SOMENTE o timestamp. Isso evita transferir todo o
        // JSON (incluindo fotos em Base64) a cada verificação.
        const updatedAt = await getCloudDataUpdatedAt(sessaoDaLeitura);

        // Só baixa o conteúdo completo quando realmente houve alteração.
        if (!updatedAt || updatedAt === ultimoServidor.current) return;
    ...
    const aoVoltarParaTela = () => {
      if (document.visibilityState === "visible") void sincronizarDeOutroDispositivo();
    };
