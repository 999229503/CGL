...
import { cloudConfigured, getSession, getCloudUpdatedAt, loadCloudData, login, logout, saveCloudData, type AuthSession, type CloudData } from "./cloud";
  ...
  useEffect(() => {
    ...
    const sincronizarDeOutroDispositivo = async () => {
      ...
      try {
        ...
        // Primeiro consulte apenas o carimbo de atualização. Isso evita
        // baixar o JSON completo (inclusive fotos em Base64) a cada ciclo.
        const updatedAtRemoto = await getCloudUpdatedAt(sessaoDaLeitura);
        ...
        if (updatedAtRemoto && updatedAtRemoto !== ultimoServidor.current) {
          // Só neste caso baixa o conteúdo completo.
          const remoto = await loadCloudData(sessaoDaLeitura);
          if (geracaoGravacaoServidor.current !== geracaoDaLeitura) return;
          if (gravacoesPendentes.current > 0 || gravacaoServidorEmAndamento.current || timerGravacaoServidor.current !== null) return;
          if (remoto.data && remoto.updatedAt === updatedAtRemoto) {
            aplicarDados(remoto.data);
            ultimoServidor.current = remoto.updatedAt;
          }
        }
