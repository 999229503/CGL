...
/* Ações individuais do Registro de pagamentos */
.registroPagamentoAcoes{
  display:flex;
  flex-direction:column;
...
.registroPagamentoAcoes .registroPagamentoValor{align-self:flex-end}
.registroVoltarBtn,.registroApagarBtn{
...
@media(max-width:700px){
  ...
  .registroPagamentoAcoes .registroPagamentoValor{align-self:flex-start}
  .registroVoltarBtn,.registroApagarBtn{width:100%}
}
...
function App() {
  ...
  const voltarPagamentoParaPendentes = async (registroId: number) => {
    ...
    if (!window.confirm(`Voltar o pagamento de ${registro.nomeFuncionario} para Pagamentos?`)) return;

    const pagamentoExistente = pagamentos.find(
      (item) =>
        item.origem === "diarias" &&
        item.funcionarioId === registro.funcionarioId &&
        item.semanaInicio === registro.semanaInicio &&
        item.semanaFim === registro.semanaFim
    );

    const novoPagamento: Pagamento = pagamentoExistente ?? {
      id: Date.now() + Math.floor(Math.random() * 100000),
      obra: "",
      descricao: `${registro.nomeFuncionario} • ${registro.diasTrabalhados} ${
        registro.diasTrabalhados === 1 ? "dia" : "dias"
      } trabalhados • Semana ${registro.semanaInicio || "-"} a ${registro.semanaFim || "-"}`,
      valor: registro.valor,
      data: registro.semanaFim || hoje,
      status: "Pendente",
      funcionarioId: registro.funcionarioId,
      semanaInicio: registro.semanaInicio || undefined,
      semanaFim: registro.semanaFim || undefined,
      diasTrabalhados: registro.diasTrabalhados,
      origem: "diarias",
    };

    const novosPagamentos = pagamentoExistente
      ? pagamentos.map((item) =>
          item.id === pagamentoExistente.id
            ? { ...item, status: "Pendente" as PagamentoStatus }
            : item
        )
    ...
    try {
      await salvarAlteracaoImediata({
        obras, tarefas, pessoas: novasPessoas, materiais, despesas,
        pagamentos: novosPagamentos, ferramentas,
      ...
      alert("Não foi possível retornar o pagamento.");
    }
  };
  ...
  const excluirRegistroPagamento = async (registroId: number) => {
    ...
    if (!window.confirm(`Apagar definitivamente o registro de ${registro.nomeFuncionario}?`)) return;
    ...
    try {
      await salvarAlteracaoImediata({
        obras, tarefas, pessoas, materiais, despesas,
        pagamentos, ferramentas,
        registrosPagamentos: novosRegistros,
      } as CloudDataComHistorico);
    } catch {
      setRegistrosPagamentos(registrosPagamentos);
