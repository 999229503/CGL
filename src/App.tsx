...
function App() {
  ...
  const dadosRef = useRef<CloudData>({
    obras, tarefas, pessoas, materiais, despesas, pagamentos, ferramentas, registrosPagamentos,
  } as CloudData);
  ...
  const salvarAlteracaoImediata = (dados: CloudData, marcarSalvamentoDireto = true): Promise<void> => {
    const dadosComHistorico = { ...(dados as object), registrosPagamentos: (dados as CloudData & { registrosPagamentos?: RegistroPagamento[] }).registrosPagamentos ?? dadosRef.current.registrosPagamentos ?? [] } as CloudData;
  ...
  const aplicarDados = (dados: CloudData) => {
    ...
    const registrosPagamentosSeguros: RegistroPagamento[] = listaSegura((dados as CloudData & { registrosPagamentos?: RegistroPagamento[] }).registrosPagamentos).map((item, indice) => ({
      id: Number(item.id) || Date.now() + indice,
  ...
  const salvarGerenciamentoPessoa = async () => {
    ...
    semanasPessoa.forEach((semana) => {
      ...
      pagamentosSemanaisAtualizados.push({
        id: existente?.id ?? Date.now() + Math.floor(Math.random() * 100000),
        obra: "",
  ...
  const alternarStatusPagamento = async (id: number) => {
    ...
    if (pagamento.origem === "diarias" && pagamento.funcionarioId && pagamento.status !== "Pago") {
      ...
      const registro: RegistroPagamento = {
        ...
        valor: pagamento.valor, diasTrabalhados: pagamento.diasTrabalhados || 0,
        semanaInicio: pagamento.semanaInicio || "", semanaFim: pagamento.semanaFim || "", dataPagamento: hoje,
      };
      ...
      try {
        await salvarAlteracaoImediata({ obras, tarefas, pessoas: novasPessoas, materiais, despesas, pagamentos: novosPagamentos, ferramentas, registrosPagamentos: novosRegistros } as CloudData);
      } catch {
    ...
    setPagamentos(novosPagamentos);
    try { await salvarAlteracaoImediata({ obras, tarefas, pessoas, materiais, despesas, pagamentos: novosPagamentos, ferramentas } as CloudData); }
...
/* Cores dos dias da semana:
   azul = trabalhou / vermelho = não trabalhou */
.diasSemanaGrid .diaTrabalho.trabalhou {
  background: #e8f2ff !important;
  border-color: #2f80ed !important;
  color: #1769c2 !important;
}
.diasSemanaGrid .diaTrabalho.naoTrabalhou {
  background: #fff0f0 !important;
  border-color: #ef5350 !important;
  color: #c62828 !important;
}
.diasSemanaGrid .diaTrabalho.naoTrabalhou b {
  color: #c62828 !important;
}
.diasSemanaGrid .diaTrabalho.trabalhou b {
  color: #1769c2 !important;
}
`}</style>
  ...
  const conteudoPagamentos = (
    <>
      ...
      <section className="panel pagamentoHistoricoPanel">
        <div className="panelHeader"><div><h2>📁 Registro de pagamentos</h2><p>Histórico dos pagamentos dos funcionários.</p></div><div className="registroPagamentoBadge">{registrosPagamentos.length} {registrosPagamentos.length === 1 ? "registro" : "registros"}</div></div>
        {registrosPagamentos.length === 0 ? <div className="registroPagamentoEmpty">📁 Nenhum pagamento arquivado ainda.<span>Ao marcar um funcionário como pago, ele aparecerá aqui.</span></div> : <div className="registroPagamentosGrid">
          {registrosPagamentos.map((r) => <article className="registroPagamentoCard" key={r.id}><div className="registroPagamentoIcon">✓</div><div className="registroPagamentoInfo"><strong>{r.nomeFuncionario}</strong><span>{r.diasTrabalhados} {r.diasTrabalhados === 1 ? "dia trabalhado" : "dias trabalhados"}</span><span>Período: {r.semanaInicio ? `${r.semanaInicio.slice(8,10)}/${r.semanaInicio.slice(5,7)}/${r.semanaInicio.slice(0,4)} a ${r.semanaFim.slice(8,10)}/${r.semanaFim.slice(5,7)}/${r.semanaFim.slice(0,4)}` : "-"}</span><small>Pago em {r.dataPagamento ? `${r.dataPagamento.slice(8,10)}/${r.dataPagamento.slice(5,7)}/${r.dataPagamento.slice(0,4)}` : "-"}</small></div><strong className="registroPagamentoValor">{dinheiro(r.valor)}</strong></article>)}
        </div>}
      </section>
    </>
  );
