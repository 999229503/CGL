...
.projectMeta{grid-template-columns:repeat(4,minmax(0,1fr))!important}
@media (max-width:760px){
  .projectMeta{grid-template-columns:repeat(2,minmax(0,1fr))!important}
}
`;
...
function App() {
  ...
  const alternarStatusPagamento = async (id: number) => {
    const novosPagamentos: Pagamento[] = pagamentos.map((pagamento): Pagamento =>
      pagamento.id === id
        ? {
            ...pagamento,
            status: (pagamento.status === "Pago" ? "Pendente" : "Pago") as PagamentoStatus,
          }
        : pagamento
    );
