...
const adicionarEtapa = () => {
  ...
  const gerarPdfMateriais = async () => {
    ...
    try {
      ...
      const azulClaro = "0.91 0.95 0.98";
      const azulMuitoClaro = "0.965 0.98 0.995";
      const texto = "0.10 0.13 0.18";
      const cinza = "0.42 0.46 0.51";
      ...
      c += linhaPdf("OBRA", M + 14, y + 17, 7, true);
      c += linhaPdf(materiaisOrdenados[0]?.obra || "Todas as obras", M + 14, y + 3, 10, true);
      ...
      const drawTableHeader = () => {
        ...
        c += linhaPdf("#", cols.no + 12, y - 14, 7.5, true);
        c += linhaPdf("MATERIAL", cols.material + 8, y - 14, 7.5, true);
        c += linhaPdf("UNIDADE", cols.unidade + 7, y - 14, 7.5, true);
        c += linhaPdf("QUANT.", cols.qtd + 4, y - 14, 7.5, true);
        c += linhaPdf("VALOR UNIT.", cols.unit + 2, y - 14, 7.5, true);
        c += linhaPdf("TOTAL", cols.total + 2, y - 14, 7.5, true);
        y -= rowH;
      };
      ...
      materiaisOrdenados.forEach((m, indice) => {
        ...
        if (indice % 2 === 0) c += retangulo(azulMuitoClaro, M, y - altura + 4, tableW, altura);
        c += linhaPdf(String(indice + 1), cols.no + 12, y - 14, 7.5, false);
      ...
      c += linhaPdf("VALOR TOTAL", 373, y - 17, 7.5, true);
      c += linhaPdf(dinheiro(total), 373, y - 40, 15, true);
      c += retangulo(azulMuitoClaro, M, 52, W - M * 2, 32, 8);
      c += linhaPdf("Materiais organizados para controle, planejamento e acompanhamento da obra.", M + 12, 71, 7.2, false);
      ...
      if (!registros.length) {
        ...
        for (const registro of registros) {
          ...
          const secao = (titulo: string, altura: number) => {
            ...
            s += linhaPdf(titulo, M + 13, y - 16, 8.5, true);
            return s;
          };
          ...
          descricao.forEach((linha, i) => {
            c += linhaPdf(linha, M + 13, y - 34 - i * 11, 8.5, false);
          });
          y -= alturaDesc + 10;
          ...
          if (imagens.length) {
            ...
            c += linhaPdf(`FOTOS DO DIA  •  ${imagens.length}`, M + 13, y - 15, 8.5, true);
            y -= 32;
            ...
            const desenharFotos = (lote: PdfImagem[], inicioGlobal: number, tituloFotos: boolean) => {
              ...
              if (tituloFotos) {
                ...
                cc += linhaPdf(`FOTOS DO DIA  •  ${imagens.length}`, M + 13, yy - 15, 8.5, true);
                yy -= 32;
              }
