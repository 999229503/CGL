...
type CloudDataComHistorico = CloudData & { registrosPagamentos?: RegistroPagamento[] };
...
function App() {
  ...
  const salvarAlteracaoImediata = (dados: CloudDataComHistorico, marcarSalvamentoDireto = true): Promise<void> => {
    const dadosComHistorico = {
      ...dados,
      registrosPagamentos: dados.registrosPagamentos ?? (dadosRef.current as CloudDataComHistorico).registrosPagamentos ?? [],
    };
    ...
    const snapshot = JSON.parse(JSON.stringify(dados)) as CloudDataComHistorico;
  ...
  const aplicarDados = (dados: CloudDataComHistorico) => {
    ...
    const registrosPagamentosSeguros: RegistroPagamento[] = listaSegura(dados.registrosPagamentos).map((item, indice) => ({
      id: Number(item.id) || Date.now() + indice,
  ...
  const adicionarPessoa = () => {
    ...
    setPessoas(novasPessoas);
    void salvarAlteracaoImediata({ obras, tarefas, pessoas: novasPessoas, materiais, despesas, pagamentos, ferramentas } as CloudDataComHistorico);
...
const adicionarEtapa = () => {
  ...
  const adicionarFerramenta = () => {
    ...
    setFerramentas(novasFerramentas);
    void salvarAlteracaoImediata({ obras, tarefas, pessoas, materiais, despesas, pagamentos, ferramentas: novasFerramentas } as CloudDataComHistorico);
