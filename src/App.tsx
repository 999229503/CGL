...
const adicionarEtapa = () => {
    ...
    try {
      ...
      materiaisOrdenados.forEach((m, indice) => {
        ...
        for (let i = 0; i < linhasDaLinha; i++) {
          const yy = y - 11 - i * 11;
          paginaConteudo += linhaPdf(obra[i] || "", margem + 7, yy, 8, false);
          paginaConteudo += linhaPdf(nome[i] || "", 178, yy, 8, i === 0);
        }

        paginaConteudo += linhaPdf(`${m.quantidade} ${m.unidade}`, 365, y - 11, 8, false);
        paginaConteudo += linhaPdf(dinheiro(m.valor), 420, y - 11, 8, false);
  ...
  const gerarPdfDiario = async () => {
    ...
    try {
      ...
      if (!registros.length) {
        paginas.push({ conteudo: linhaPdf("CGL - Gerenciamento de Obras", 42, 800, 18, true) + linhaPdf("DIÁRIO DE OBRA", 42, 770, 14, true) + linhaPdf("Nenhum registro cadastrado.", 42, 735, 10), imagens: [] });
      } else {
        for (const registro of registros) {
          ...
          const linhas: string[] = [
            "CGL - Gerenciamento de Obras",
            "DIÁRIO DE OBRA",
            `Data: ${registro.data ? `${registro.data.slice(8,10)}/${registro.data.slice(5,7)}/${registro.data.slice(0,4)}` : "-"}`,
            `Obra: ${registro.obra}`,
            `Título: ${registro.titulo}`,
            registro.etapa ? `Etapa / Serviço: ${registro.etapa}` : "",
            "O QUE FOI FEITO",
            ...quebrarTextoPdf(registro.descricao, 88),
            registro.observacao ? "OBSERVAÇÕES" : "",
            ...(registro.observacao ? quebrarTextoPdf(registro.observacao, 88) : []),
            imagens.length ? `FOTOS DO DIA: ${imagens.length}` : "FOTOS DO DIA: nenhuma",
          ].filter(Boolean);
          let y = 800;
          let conteudo = "";
          linhas.forEach((linha, i) => {
            const ehTitulo = i === 0 || i === 1;
            const secao = linha === "O QUE FOI FEITO" || linha === "OBSERVAÇÕES" || linha.startsWith("FOTOS DO DIA:");
            const tamanho = ehTitulo ? (i === 0 ? 18 : 14) : secao ? 10 : 9;
            conteudo += linhaPdf(linha, 42, y, tamanho, ehTitulo || secao);
            y -= ehTitulo ? 25 : 15;
          });
          // Fotos ficam na parte inferior da página, até três por página. Se houver muitas,
          // criamos páginas extras para não esmagar as informações do registro.
          if (imagens.length <= 3) {
            const larguraMax = 165;
            const alturaMax = 170;
            imagens.forEach((imagem, i) => {
              const escala = Math.min(larguraMax / imagem.largura, alturaMax / imagem.altura);
              const w = imagem.largura * escala;
              const h = imagem.altura * escala;
              const x = 42 + i * 175;
              const yImg = 55;
              conteudo += `q ${w.toFixed(2)} 0 0 ${h.toFixed(2)} ${x.toFixed(2)} ${yImg.toFixed(2)} cm /Im${i + 1} Do Q\n`;
            });
            paginas.push({ conteudo, imagens });
          } else {
            paginas.push({ conteudo, imagens: [] });
            for (let inicio = 0; inicio < imagens.length; inicio += 3) {
              const lote = imagens.slice(inicio, inicio + 3);
              let fotosConteudo = linhaPdf("DIÁRIO DE OBRA • FOTOS", 42, 800, 14, true) + linhaPdf(`Obra: ${registro.obra} • ${registro.titulo}`, 42, 775, 9, true);
              lote.forEach((imagem, i) => {
                const escala = Math.min(165 / imagem.largura, 220 / imagem.altura);
                const w = imagem.largura * escala;
                const h = imagem.altura * escala;
                const x = 42 + i * 175;
                const yImg = 120;
                fotosConteudo += `q ${w.toFixed(2)} 0 0 ${h.toFixed(2)} ${x.toFixed(2)} ${yImg.toFixed(2)} cm /Im${i + 1} Do Q\n`;
                if (imagem.nome) fotosConteudo += linhaPdf(imagem.nome, x, 105, 7);
              });
              paginas.push({ conteudo: fotosConteudo, imagens: lote });
            }
          }
        }
      }
      const blob = criarPdf(paginas);
      const url = URL.createObjectURL(blob);
      setPdfGerado((anterior) => { if (anterior) URL.revokeObjectURL(anterior.url); return { url, nome: `CGL-Diario-de-Obra-${hoje}.pdf`, tipo: "diario" }; });
      const link = document.createElement("a"); link.href = url; link.download = `CGL-Diario-de-Obra-${hoje}.pdf`; document.body.appendChild(link); link.click(); link.remove();
