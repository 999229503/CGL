...
const estilos = `
*{box-sizing:border-box}html,body,#root{margin:0;width:100%;min-height:100%;}body{font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#07111f;color:#e7eef8}button,input,select{font:inherit}button{cursor:pointer;-webkit-tap-highlight-color:transparent}
...
.app{min-height:100vh;display:flex;background:radial-gradient(circle at 75% 0%,rgba(22,119,255,.08),transparent 34%),var(--bg);overflow-x:hidden}
...
@media(max-width:650px){.sidebar{position:fixed;left:0;right:0;top:auto;bottom:0;width:100%;height:68px;padding:7px 8px;background:#081523;border-right:0;border-top:1px solid #1b334b;box-shadow:0 -12px 35px rgba(0,0,0,.35)}.logo,.sidebarFooter{display:none}.nav{padding:0;display:grid;grid-template-columns:repeat(8,1fr);gap:3px;overflow:hidden;width:100%}.nav button{height:54px;border-radius:9px;padding:0;gap:2px;flex-direction:column;font-size:9px}.navIcon{width:20px;height:20px}.navLabel{display:block;font-size:8px;color:inherit}.nav button.active{box-shadow:inset 0 2px 0 #2388ff}.main{margin-left:0;width:100%;padding:12px 10px 84px}.topbar{margin-bottom:13px;height:42px}.searchBox{max-width:none;height:36px}.bell{width:32px;height:32px}.dashboardGrid{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.card{padding:11px;gap:8px;border-radius:11px}.cardIcon{width:33px;height:33px;border-radius:9px;font-size:15px}.cardInfo span{font-size:9px}.cardInfo strong{font-size:14px}.dashboardHero{gap:9px}.projectCard{min-height:190px}.projectContent{padding:14px}.projectContent h3{font-size:15px}.projectMeta{gap:15px}.dashboardProgress{padding:15px;gap:14px}.donut{width:92px;height:92px}.donut:after{width:64px;height:64px}.donut span{font-size:18px}.legend{gap:7px}.legendRow{font-size:9px}.dashboardBottom{gap:9px}.panel{padding:13px;border-radius:11px}.panelHeader h2{font-size:14px}.panelHeader p{font-size:10px}.formGrid{grid-template-columns:1fr}.field.full{grid-column:auto}.itemCard{align-items:flex-start;flex-direction:column}.actions{width:100%}.actions .status{flex:1}.photoTools{grid-template-columns:1fr}.photoGrid{grid-template-columns:1fr 1fr}.photoCard img{height:115px}header{flex-wrap:wrap;gap:7px;margin-bottom:12px}header h1{font-size:21px}header .primary{width:auto}.modalOverlay{align-items:flex-start;padding:8px}.modal{margin-top:3vh}.modalBody{padding:15px}.equipeGrid{grid-template-columns:1fr}}
...
const adicionarEtapa = () => {
  ...
  const despesasPorCategoria = useMemo(() => {
    ...
    despesas.forEach((d) => { mapa[d.categoria || "Outros"] = (mapa[d.categoria || "Outros"] || 0) + Number(d.valor || 0); });
    if (totalMateriais > 0) mapa.Material = (mapa.Material || 0) + totalMateriais;
    return Object.entries(mapa).sort((a,b) => b[1]-a[1]).slice(0,5);
  }, [despesas,totalMateriais]);
  const maiorCategoria = Math.max(1, ...despesasPorCategoria.map(([,v])=>v));
  const atividades = [
    ...despesas.slice(-3).reverse().map((d) => ({icon:"$", title:"Despesa registrada", text:`${d.descricao} • ${dinheiro(d.valor)}`, date:d.data || "Hoje"})),
    ...materiais.slice(-2).reverse().map((m) => ({icon:"▣", title:"Material adicionado", text:`${m.nome} • ${m.quantidade} ${m.unidade}`, date:"Hoje"})),
    ...pagamentos.slice(-2).reverse().map((p) => ({icon:"✓", title:"Pagamento registrado", text:`${p.descricao} • ${dinheiro(p.valor)}`, date:p.data || "Hoje"})),
  ].slice(0,5);
  ...
  const conteudoDashboard = (
    <>
      ...
      <div className="grid dashboardGrid">
        ...
        <button className="card clickable dashboardCard" onClick={() => setAba("Pagamentos")}><div className="cardIcon">$</div><div className="cardInfo"><span>Investimento total</span><strong>{dinheiro(totalOrcamento)}</strong></div></button>
      </div>
      <div className="dashboardHero">
        <section className="projectCard">
          ...
          <div className="projectContent">
            {obraDestaque ? <>
              ...
              <div style={{marginTop:12,maxWidth:420}}><BarraProgresso valor={progressoObra(obraDestaque)}/></div>
              <div className="projectMeta"><div><span>Orçamento</span><strong>{dinheiro(obraDestaque.orcamento)}</strong></div><div><span>Início</span><strong>{obraDestaque.inicio || "-"}</strong></div><div><span>Previsão</span><strong>{obraDestaque.previsao || "-"}</strong></div></div>
        ...
        <section className="dashboardProgress">
          ...
          <div className="legend"><strong style={{fontSize:13}}>Progresso da obra</strong><div className="legendRow"><i className="dot blue"/> Concluído <b style={{marginLeft:"auto",color:"#dbe7f2"}}>{Math.round(progressoGeral)}%</b></div><div className="legendRow"><i className="dot green"/> Em andamento <b style={{marginLeft:"auto",color:"#dbe7f2"}}>{obras.filter(o=>o.status==="Em andamento").length}</b></div><div className="legendRow"><i className="dot orange"/> Pendente <b style={{marginLeft:"auto",color:"#dbe7f2"}}>{obras.filter(o=>o.status==="Pendente").length}</b></div></div>
        </section>
      </div>
      <div className="dashboardBottom">
        <section className="panel" style={{margin:0}}><div className="panelHeader"><div><h2>Despesas por categoria</h2><p>Distribuição dos gastos registrados.</p></div><button className="secondary" onClick={()=>setAba("Despesas")}>Ver detalhes</button></div><div className="chartBars">{(despesasPorCategoria.length?despesasPorCategoria:[['Sem dados',0] as [string,number]]).map(([nome,valor])=><div className="barCol" key={nome}><span className="barValue">{valor?dinheiro(valor):"-"}</span><div className="bar" style={{height:`${Math.max(7,(valor/maiorCategoria)*105)}px`}}/><span className="barLabel">{nome}</span></div>)}</div></section>
        <section className="panel" style={{margin:0}}><div className="panelHeader"><div><h2>Últimas atividades</h2><p>Movimentações recentes do sistema.</p></div></div><div className="activityList">{atividades.length?atividades.map((a,i)=><div className="activity" key={i}><div className="activityIcon">{a.icon}</div><div><strong>{a.title}</strong><span>{a.text} • {a.date}</span></div></div>):<div className="empty" style={{padding:20}}>Nenhuma atividade recente.</div>}</div></section>
      </div>
    </>
  );
        ...
        <>
          ...
          <div className="authScreen">
            <div className="authCard">
              ...
              <p>Seus dados continuam salvos. Recarregue a página para tentar novamente.</p>
