...
import { cloudConfigured, deleteImage, getSession, getSignedImageUrl, loadCloudData, login, logout, saveCloudData, uploadImageDataUrl, type AuthSession, type CloudData } from "./cloud";
...
function App() {
  ...
  const [pessoaFotoStoragePath, setPessoaFotoStoragePath] = useState<string | undefined>(undefined);
  ...
  const aplicarDados = (dados: CloudDataComHistorico) => {
    ...
    const obrasSeguras: Obra[] = listaSegura(dados?.obras).map((obra) => ({
      ...
      fotos: Array.isArray(obra.fotos)
        ? obra.fotos.filter((f): f is Record<string, unknown> => !!f && typeof f === "object").map((foto) => ({
            ...
            url: String(foto.url ?? ""),
            storagePath: foto.storagePath ? String(foto.storagePath) : undefined,
          })).filter((foto) => foto.url || foto.storagePath)
        : [],
    }));
    ...
    const pessoasSeguras: Pessoa[] = listaSegura(dados?.pessoas).map((item) => ({
      ...
      fotoStoragePath: item.fotoStoragePath ? String(item.fotoStoragePath) : undefined,
  ...
  const prepararFotosCloud = async (sessaoAtual: AuthSession, dados: CloudDataComHistorico): Promise<{ dados: CloudDataComHistorico; alterado: boolean }> => {
    const copia = JSON.parse(JSON.stringify(dados)) as CloudDataComHistorico;
    let alterado = false;

    const prepararFoto = async (foto: Record<string, unknown>, pathFallback: string) => {
      const path = foto.storagePath ? String(foto.storagePath) : pathFallback;
      const url = String(foto.url || "");
      if (url.startsWith("data:image/")) {
        const storagePath = path;
        await uploadImageDataUrl(sessaoAtual, url, storagePath);
        foto.storagePath = storagePath;
        foto.url = await getSignedImageUrl(sessaoAtual, storagePath);
        alterado = true;
        return;
      }
      if (foto.storagePath) {
        try { foto.url = await getSignedImageUrl(sessaoAtual, String(foto.storagePath)); }
        catch (erro) { console.warn("Não foi possível gerar URL da foto:", erro); }
      }
    };

    for (const obra of (copia.obras || []) as Record<string, unknown>[]) {
      const obraId = Number(obra.id);
      for (const foto of (Array.isArray(obra.fotos) ? obra.fotos : []) as Record<string, unknown>[]) {
        await prepararFoto(foto, `${sessaoAtual.user.id}/obras/${obraId}/${Number(foto.id)}.jpg`);
      }
    }
    for (const pessoa of (copia.pessoas || []) as Record<string, unknown>[]) {
      const url = String(pessoa.foto || "");
      if (url.startsWith("data:image/")) {
        const path = `${sessaoAtual.user.id}/pessoas/${Number(pessoa.id)}.jpg`;
        await uploadImageDataUrl(sessaoAtual, url, path);
        pessoa.fotoStoragePath = path;
        pessoa.foto = await getSignedImageUrl(sessaoAtual, path);
        alterado = true;
      } else if (pessoa.fotoStoragePath) {
        try { pessoa.foto = await getSignedImageUrl(sessaoAtual, String(pessoa.fotoStoragePath)); } catch (erro) { console.warn("Não foi possível gerar URL da foto do funcionário:", erro); }
      }
    }
    for (const diario of (copia.diarioObra || []) as Record<string, unknown>[]) {
      const diarioId = Number(diario.id);
      for (const foto of (Array.isArray(diario.fotos) ? diario.fotos : []) as Record<string, unknown>[]) {
        await prepararFoto(foto, `${sessaoAtual.user.id}/diario/${diarioId}/${Number(foto.id)}.jpg`);
      }
    }
    return { dados: copia, alterado };
  };
    ...
    (async () => {
      ...
      if (atual) {
        ...
        try {
          ...
          if (remoto.data) {
            const preparado = await prepararFotosCloud(atual, remoto.data as CloudDataComHistorico);
            aplicarDados(preparado.dados);
            if (preparado.alterado) await saveCloudData(atual, preparado.dados);
            ultimoServidor.current = remoto.updatedAt;
          } else {
            const preparado = await prepararFotosCloud(atual, dadosAtuais());
            aplicarDados(preparado.dados);
            const stamp = await comTempoLimite(saveCloudData(atual, preparado.dados), 15000);
            ultimoServidor.current = stamp;
          }
          setCloudPronto(true);
        } catch (erro) {
    ...
    const sincronizarDeOutroDispositivo = async () => {
      ...
      try {
        ...
        if (remoto.data && remoto.updatedAt && remoto.updatedAt !== ultimoServidor.current) {
          const preparado = await prepararFotosCloud(sessaoDaLeitura, remoto.data as CloudDataComHistorico);
          aplicarDados(preparado.dados);
          if (preparado.alterado) await saveCloudData(sessaoDaLeitura, preparado.dados);
          ultimoServidor.current = remoto.updatedAt;
        }
      } catch (erro) {
  ...
  const entrarNoSistema = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (remoto.data) {
        const preparado = await prepararFotosCloud(novaSessao, remoto.data as CloudDataComHistorico);
        aplicarDados(preparado.dados);
        if (preparado.alterado) await saveCloudData(novaSessao, preparado.dados);
        ultimoServidor.current = remoto.updatedAt;
      } else {
        const preparado = await prepararFotosCloud(novaSessao, dadosAtuais());
        aplicarDados(preparado.dados);
        const stamp = await comTempoLimite(saveCloudData(novaSessao, preparado.dados), 15000);
        ultimoServidor.current = stamp;
      }
  ...
  const adicionarPessoa = async () => {
    ...
    const pessoaId = pessoaEditandoId ?? novoId();
    let fotoFinal = pessoaFoto || pessoaAnterior?.foto || "";
    let fotoPathFinal = pessoaFotoStoragePath || pessoaAnterior?.fotoStoragePath;
    if (pessoaFoto.startsWith("data:image/")) {
      const sessaoAtual = sessaoRef.current;
      if (!sessaoAtual) { alert("Sua sessão online expirou. Entre novamente para salvar a foto."); return; }
      fotoPathFinal = `${sessaoAtual.user.id}/pessoas/${pessoaId}.jpg`;
      await uploadImageDataUrl(sessaoAtual, pessoaFoto, fotoPathFinal);
      fotoFinal = await getSignedImageUrl(sessaoAtual, fotoPathFinal);
    }
    ...
    try {
      ...
      setPessoaForm({ nome: "", funcao: "", telefone: "", cpf: "", endereco: "" });
      setPessoaFoto("");
      setPessoaFotoStoragePath(undefined);
      setPessoaEditandoId(null);
      setModal(null);
    } catch (erro) {
  ...
  const editarPessoa = (id: number) => {
    ...
    setPessoaFotoStoragePath(p.fotoStoragePath);
...
const adicionarEtapa = () => {
  ...
  const adicionarFoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const sessaoAtual = sessaoRef.current;
      if (!sessaoAtual) throw new Error("Sua sessão online expirou. Entre novamente para adicionar fotos.");
      const fotoId = novoId();
      const storagePath = `${sessaoAtual.user.id}/obras/${obraAtual.id}/${fotoId}.jpg`;
      await uploadImageDataUrl(sessaoAtual, url, storagePath);
      const imagemUrl = await getSignedImageUrl(sessaoAtual, storagePath);
      const novasObras = obras.map((obra) =>
  ...
  const excluirFoto = (obraId: number, fotoId: number) => {
    ...
    const fotoExcluida = obras.find((obra) => obra.id === obraId)?.fotos?.find((foto) => foto.id === fotoId);
    const novasObras = obras.map((obra) =>
      obra.id === obraId
    ...
    setObras(novasObras);
    if (fotoExcluida?.storagePath && sessaoRef.current) {
      void deleteImage(sessaoRef.current, fotoExcluida.storagePath).catch((erro) => console.warn("Não foi possível remover a imagem do Storage:", erro));
    }
    void salvarAlteracaoImediata({
      obras: novasObras,
      tarefas,
  ...
  const adicionarFotoDiario = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const sessaoAtual = sessaoRef.current;
      if (!sessaoAtual) throw new Error("Sua sessão online expirou. Entre novamente para adicionar fotos.");
      const diarioId = diarioEditandoId ?? novoId();
      const fotoId = novoId();
      const storagePath = `${sessaoAtual.user.id}/diario/${diarioId}/${fotoId}.jpg`;
      await uploadImageDataUrl(sessaoAtual, url, storagePath);
      const imagemUrl = await getSignedImageUrl(sessaoAtual, storagePath);
      ...
      if (diarioEditandoId === null) {
        ...
        setDiarioObra(novosRegistros);
        void salvarAlteracaoImediata({ obras, tarefas, pessoas, materiais, despesas, pagamentos, ferramentas, registrosPagamentos, diarioObra: novosRegistros } as CloudDataComHistorico);
      }
  ...
  const excluirFotoDiario = (registroId: number, fotoId: number) => {
    ...
    const fotoExcluida = diarioObra.find((registro) => registro.id === registroId)?.fotos?.find((foto) => foto.id === fotoId);
    const novosRegistros = diarioObra.map((registro) =>
