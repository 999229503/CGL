...
function App() {
  ...
  const semanasPessoa = useMemo(() => {
    // As semanas adicionadas ficam vinculadas ao funcionário.
    // Assim, mesmo depois de fechar o modal/recarregar o app, elas continuam existindo.
    if (semanasPessoaInicios.length > 0) {
      return semanasPessoaInicios.map((inicioChave) => {
        ...
        return nomes.map((nome, index) => {
          ...
          data.setDate(inicio.getDate() + index);
          const chave = `${data.getFullYear()}-${String(data.getMonth() + 1).padStart(2, "0")}-${String(data.getDate()).padStart(2, "0")}`;
          return { nome, chave, data: data.toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" }) };
        });
      });
    }
    return Array.from({ length: semanasPessoaVisiveis }, (_, index) => gerarSemanaPessoa(index));
  ...
  const alternarStatusPagamento = async (id: number) => {
    const novosPagamentos = pagamentos.map((pagamento) =>
      pagamento.id === id
        ? {
            ...pagamento,
            status: pagamento.status === "Pago" ? "Pendente" : "Pago",
          }
        : pagamento
    );
            ...
            <table>
              ...
              <tbody>
                {pagamentos.map((pagamento) => (
                  <tr key={pagamento.id}>
                    <td>{pagamento.origem === "diarias" ? "Funcionários" : (pagamento.obra || "-")}</td>
                    <td>
                      <strong>{pagamento.origem === "diarias" && pagamento.funcionarioId
                        ? (pessoas.find((p) => p.id === pagamento.funcionarioId)?.nome || pagamento.descricao.split(" • ")[0])
                        : pagamento.descricao}</strong>
                      {pagamento.origem === "diarias" && (
                        <div className="muted">
                          {pagamento.diasTrabalhados || 0} {(pagamento.diasTrabalhados || 0) === 1 ? "dia" : "dias"} trabalhados
                          {pagamento.semanaInicio && pagamento.semanaFim
                            ? ` • ${pagamento.semanaInicio.slice(8, 10)}/${pagamento.semanaInicio.slice(5, 7)} a ${pagamento.semanaFim.slice(8, 10)}/${pagamento.semanaFim.slice(5, 7)}`
                            : ""}
                        </div>
                      )}
                    </td>
                    ...
                    <td>
                      {pagamento.origem === "diarias" ? (
                        <button
                          type="button"
                          className={`pagamentoStatusBtn ${
                            pagamento.status === "Pago" ? "pago" : "pendente"
                          }`}
                          onClick={() => alternarStatusPagamento(pagamento.id)}
                    ...
                    <td>
                      {pagamento.origem === "diarias" ? (
                        <div className="mobileActionButtons">
                          <button
                            className="danger"
                            onClick={() => excluir("pagamento", pagamento.id)}
                          >
                            🗑️ Apagar
                          </button>
                        </div>
          ...
          {semanasPessoa.map((semana, indiceSemana) => (
            <div className="semanaPessoaBloco" key={`semana-${indiceSemana}`}>
              <div className="sectionTitle">
                🗓️ Domingo a sábado • {semana[0].data} a {semana[6].data}
              </div>
