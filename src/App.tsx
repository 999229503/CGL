...
function App() {
  ...
  const abrirDetalhesPessoa = (id: number) => {
    if (!pessoas.some((item) => item.id === id)) return;
    setPessoaSelecionadaId(id);
    setModal("detalhesPessoa");
  };
...
const adicionarEtapa = () => {
  ...
  const conteudoFuncionarios = (
    <>
      ...
      <section className="panel funcionariosPanel">
        <div className="funcionariosResumo">
          <div className="funcionariosTitulo">
            <div className="funcionariosTituloIcon">👥</div>
            <div>
              <h2>Equipe de funcionários</h2>
              <p>Consulte rapidamente sua equipe e abra o perfil completo de cada funcionário.</p>
            </div>
          </div>

          <div className="funcionariosIndicadores">
            <div className="funcionarioIndicador">
              <span>Total</span>
              <strong>{pessoas.length}</strong>
            </div>
            <div className="funcionarioIndicador ativo">
              <span>Ativos</span>
              <strong>{pessoas.length}</strong>
            </div>
            <div className="funcionarioIndicador diaria">
              <span>Diárias cadastradas</span>
              <strong>{pessoas.filter((p) => Number(p.diaria || 0) > 0).length}</strong>
            </div>
          </div>
        </div>
        ...
        {pessoas.length === 0 ? (
          ...
          <div className="funcionariosTabelaWrap">
            <table className="funcionariosTabela">
              <thead>
                <tr>
                  <th>Funcionário</th>
                  <th>Função</th>
                  ...
                  <th>Diária</th>
                  <th>Status</th>
                  <th className="funcionariosAcaoCol">Ações</th>
                </tr>
              </thead>
              <tbody>
                {pessoas.map((pessoa) => {
                  const obraPessoa = obras.find((obra) => (obra.equipe || []).some((id) => Number(id) === Number(pessoa.id)));
                  return (
                    <tr key={pessoa.id}>
                      <td>
                        <button
                          type="button"
                          className="funcionarioNomeBtn"
                          onClick={() => abrirDetalhesPessoa(pessoa.id)}
                        >
                          <span className="funcionarioAvatar">👷</span>
                          <span>
                            <strong>{pessoa.nome}</strong>
                            <small>CPF: {pessoa.cpf || "Não cadastrado"}</small>
                          </span>
                        </button>
                      </td>
                      <td><span className="funcionarioTexto">{pessoa.funcao || "Não informada"}</span></td>
                      <td><span className="funcionarioTexto">{obraPessoa?.nome || "Sem obra"}</span></td>
                      <td><span className="funcionarioTexto">{pessoa.telefone || "Não informado"}</span></td>
                      <td><strong className="funcionarioDiaria">{dinheiro(pessoa.diaria || 0)}</strong></td>
                      <td><span className="funcionarioStatus ativo">Ativo</span></td>
                      <td>
                        <div className="funcionarioAcoes">
                          <button type="button" className="funcionarioVerBtn" onClick={() => abrirDetalhesPessoa(pessoa.id)}>Ver perfil</button>
                          <button type="button" className="funcionarioEditarBtn" onClick={() => editarPessoa(pessoa.id)} aria-label={`Editar ${pessoa.nome}`}>✏️</button>
                          <button type="button" className="funcionarioExcluirBtn" onClick={() => excluir("pessoa", pessoa.id)} aria-label={`Excluir ${pessoa.nome}`}>🗑️</button>
                        </div>
                      </td>
                    </tr>
                  );
            ...
            <div className="funcionariosMobileList">
              {pessoas.map((pessoa) => {
                const obraPessoa = obras.find((obra) => (obra.equipe || []).some((id) => Number(id) === Number(pessoa.id)));
                return (
                  <article className="funcionarioMobileCard" key={`mobile-${pessoa.id}`}>
                    <button type="button" className="funcionarioMobilePrincipal" onClick={() => abrirDetalhesPessoa(pessoa.id)}>
                      <span className="funcionarioAvatar grande">👷</span>
                      <span className="funcionarioMobileIdentidade">
                        <strong>{pessoa.nome}</strong>
                        <small>{pessoa.funcao || "Função não informada"}</small>
                      </span>
                      <span className="funcionarioStatus ativo">Ativo</span>
                    </button>

                    <div className="funcionarioMobileDados">
                      <div><span>Obra</span><strong>{obraPessoa?.nome || "Sem obra"}</strong></div>
                      <div><span>Telefone</span><strong>{pessoa.telefone || "Não informado"}</strong></div>
                      <div><span>Diária</span><strong>{dinheiro(pessoa.diaria || 0)}</strong></div>
                    </div>

                    <div className="funcionarioMobileAcoes">
                      <button type="button" className="funcionarioVerBtn" onClick={() => abrirDetalhesPessoa(pessoa.id)}>Abrir perfil</button>
                      <button type="button" className="funcionarioEditarBtn" onClick={() => editarPessoa(pessoa.id)}>✏️ Editar</button>
                      <button type="button" className="funcionarioExcluirBtn" onClick={() => excluir("pessoa", pessoa.id)}>🗑️</button>
                    </div>
                  </article>
                );
              })}
  ...
  return (
    <>
      ...
      {modal === "detalhesPessoa" && pessoaSelecionadaId !== null && (() => {
        const pessoa = pessoas.find((item) => item.id === pessoaSelecionadaId);
        if (!pessoa) return null;
        const obraPessoa = obras.find((obra) => (obra.equipe || []).some((id) => Number(id) === Number(pessoa.id)));
        const diasMarcados = Object.values(pessoa.diasTrabalhados || {}).filter(Boolean).length;
        const semanas = pessoa.semanasGerenciadas?.length || 0;

        return (
          <Modal
            titulo="Perfil do funcionário"
            fechar={() => { setModal(null); setPessoaSelecionadaId(null); }}
            largura={900}
          >
            <div className="perfilFuncionario">
              <div className="perfilFuncionarioTopo">
                <div className="perfilFuncionarioIdentidade">
                  <span className="perfilFuncionarioAvatar">👷</span>
                  <div>
                    <div className="perfilFuncionarioNomeLinha">
                      <h2>{pessoa.nome}</h2>
                      <span className="funcionarioStatus ativo">Ativo</span>
                    </div>
                    <p>{pessoa.funcao || "Função não informada"}</p>
                  </div>
                </div>

                <div className="perfilFuncionarioAcoes">
                  <button
                    type="button"
                    className="funcionarioEditarPrincipal"
                    onClick={() => {
                      setModal(null);
                  ...
                  <button
                    type="button"
                    className="funcionarioGerenciarBtn"
                    onClick={() => {
                      setModal(null);
                      abrirGerenciamentoPessoa(pessoa.id);
                    }}
                  >
                    📅 Gerenciar dias
                  </button>
              ...
              <div className="perfilFuncionarioResumo">
                <div><span>Obra atual</span><strong>{obraPessoa?.nome || "Sem obra vinculada"}</strong></div>
                <div><span>Diária</span><strong>{dinheiro(pessoa.diaria || 0)}</strong></div>
                <div><span>Telefone</span><strong>{pessoa.telefone || "Não informado"}</strong></div>
                <div><span>Chave Pix</span><strong>{pessoa.pix || "Não cadastrada"}</strong></div>
              </div>

              <div className="perfilFuncionarioAbas">
                <span className="ativa">Dados pessoais</span>
              ...
              <div className="perfilFuncionarioGrid">
                <div className="perfilInfoCard">
                  <div className="perfilInfoCabecalho"><span>👤</span><strong>Dados pessoais</strong></div>
                  <div className="perfilCampo"><span>Nome completo</span><strong>{pessoa.nome}</strong></div>
                  <div className="perfilCampo"><span>CPF</span><strong>{pessoa.cpf || "Não cadastrado"}</strong></div>
                  <div className="perfilCampo"><span>Telefone</span><strong>{pessoa.telefone || "Não informado"}</strong></div>
                  <div className="perfilCampo"><span>Endereço</span><strong>{pessoa.endereco || "Não cadastrado"}</strong></div>
                </div>

                <div className="perfilInfoCard">
                  <div className="perfilInfoCabecalho"><span>💼</span><strong>Dados profissionais</strong></div>
                  <div className="perfilCampo"><span>Função</span><strong>{pessoa.funcao || "Não informada"}</strong></div>
                  <div className="perfilCampo"><span>Obra vinculada</span><strong>{obraPessoa?.nome || "Sem obra vinculada"}</strong></div>
                  <div className="perfilCampo"><span>Valor da diária</span><strong>{dinheiro(pessoa.diaria || 0)}</strong></div>
                  <div className="perfilCampo"><span>Tipo de Pix</span><strong>{pessoa.tipoPix || "Aleatória"}</strong></div>
                </div>

                <div className="perfilInfoCard">
                  <div className="perfilInfoCabecalho"><span>🗓️</span><strong>Controle de trabalho</strong></div>
                  <div className="perfilMiniResumo"><strong>{diasMarcados}</strong><span>dias marcados</span></div>
                  <div className="perfilMiniResumo"><strong>{semanas}</strong><span>semanas gerenciadas</span></div>
                  <button type="button" className="perfilGerenciarLargo" onClick={() => { setModal(null); abrirGerenciamentoPessoa(pessoa.id); }}>
                    Abrir gerenciamento de dias
                  </button>
                </div>
              </div>

              <div className="perfilFuncionarioRodape">
                <button type="button" className="cancel" onClick={() => { setModal(null); setPessoaSelecionadaId(null); }}>Fechar</button>
                <button type="button" className="primary" onClick={() => { setModal(null); setPessoaSelecionadaId(null); editarPessoa(pessoa.id); }}>✏️ Editar informações</button>
              </div>
            </div>
          </Modal>
        );
...
/* CGL • Funcionários — tabela desktop + cards mobile + perfil */
.funcionariosPanel{overflow:hidden}
.funcionariosResumo{display:flex;align-items:center;justify-content:space-between;gap:18px;margin-bottom:18px;padding:16px 18px;border:1px solid #e4eaf1;border-radius:14px;background:#fff}
.funcionariosTitulo{display:flex;align-items:center;gap:12px;min-width:0}
.funcionariosTituloIcon{width:42px;height:42px;border-radius:12px;display:grid;place-items:center;background:#eef6ff;color:#087cf0;font-size:21px;flex:0 0 auto}
.funcionariosTitulo h2{margin:0;font-size:17px;color:#172033}
.funcionariosTitulo p{margin:4px 0 0;color:#657286;font-size:11px}
.funcionariosIndicadores{display:flex;gap:9px;flex-wrap:wrap}
.funcionarioIndicador{min-width:92px;padding:9px 12px;border:1px solid #e5ebf2;border-radius:11px;background:#fafcff;text-align:center}
.funcionarioIndicador span{display:block;font-size:9px;color:#657286;font-weight:700;text-transform:uppercase;letter-spacing:.3px}
.funcionarioIndicador strong{display:block;margin-top:2px;font-size:18px;color:#172033}
.funcionarioIndicador.ativo strong{color:#15965c}
.funcionarioIndicador.diaria strong{color:#087cf0}
.funcionariosTabelaWrap{width:100%;overflow-x:auto;border:1px solid #e1e8f0;border-radius:14px;background:#fff}
.funcionariosTabela{width:100%;min-width:900px;border-collapse:separate;border-spacing:0}
.funcionariosTabela th{padding:11px 12px;background:#f7f9fc;border-bottom:1px solid #dfe6ee;color:#536176;font-size:9px;text-transform:uppercase;letter-spacing:.35px;text-align:left;white-space:nowrap}
.funcionariosTabela td{padding:11px 12px;border-bottom:1px solid #edf1f5;color:#253044;font-size:11px;vertical-align:middle}
.funcionariosTabela tbody tr:last-child td{border-bottom:0}
.funcionariosTabela tbody tr:hover td{background:#fbfdff}
.funcionarioNomeBtn{display:flex;align-items:center;gap:10px;padding:0;border:0;background:transparent;text-align:left;color:#172033;cursor:pointer}
.funcionarioNomeBtn>span:last-child{display:flex;flex-direction:column;min-width:0}
.funcionarioNomeBtn strong{font-size:12px;white-space:nowrap}
.funcionarioNomeBtn small{margin-top:2px;color:#7a8798;font-size:9px;white-space:nowrap}
.funcionarioAvatar{width:34px;height:34px;border-radius:50%;display:grid;place-items:center;background:#eef5fc;border:1px solid #d9e7f5;font-size:17px;flex:0 0 auto}
.funcionarioAvatar.grande{width:40px;height:40px}
.funcionarioTexto{color:#46546a}
.funcionarioDiaria{color:#172033;white-space:nowrap}
.funcionarioStatus{display:inline-flex;align-items:center;justify-content:center;padding:5px 9px;border-radius:999px;font-size:9px;font-weight:800;white-space:nowrap}
.funcionarioStatus.ativo{background:#e8f8ef;color:#138653;border:1px solid #c9efd9}
.funcionariosAcaoCol{text-align:right!important}
.funcionarioAcoes{display:flex;justify-content:flex-end;align-items:center;gap:5px}
.funcionarioAcoes button,.funcionarioMobileAcoes button{min-height:31px;border-radius:8px;font-size:10px;font-weight:800;cursor:pointer}
.funcionarioVerBtn{border:1px solid #cfe0f2;background:#f5f9fe;color:#0873d7;padding:7px 10px}
.funcionarioEditarBtn{border:1px solid #cfe0f2;background:#eaf4ff;color:#0873d7;padding:7px 9px}
.funcionarioExcluirBtn{border:1px solid #f3d0d0;background:#fff5f5;color:#d33b3b;padding:7px 9px}
.funcionariosMobileList{display:none}
.perfilFuncionario{color:#172033}
.perfilFuncionarioTopo{display:flex;align-items:center;justify-content:space-between;gap:18px;padding:4px 2px 17px;border-bottom:1px solid #e5ebf2}
.perfilFuncionarioIdentidade{display:flex;align-items:center;gap:13px;min-width:0}
.perfilFuncionarioAvatar{width:64px;height:64px;border-radius:16px;display:grid;place-items:center;background:#eef6ff;border:1px solid #d7e7f7;font-size:31px;flex:0 0 auto}
.perfilFuncionarioNomeLinha{display:flex;align-items:center;gap:9px;flex-wrap:wrap}
.perfilFuncionarioNomeLinha h2{margin:0;color:#172033;font-size:20px}
.perfilFuncionarioIdentidade p{margin:4px 0 0;color:#627086;font-size:11px}
.perfilFuncionarioAcoes{display:flex;gap:7px;flex-wrap:wrap;justify-content:flex-end}
.funcionarioEditarPrincipal,.funcionarioGerenciarBtn{border-radius:9px;padding:9px 12px;font-size:10px;font-weight:800;cursor:pointer}
.funcionarioEditarPrincipal{border:1px solid #087cf0;background:#087cf0;color:#fff}
.funcionarioGerenciarBtn{border:1px solid #cfe0f2;background:#f6faff;color:#086fca}
.perfilFuncionarioResumo{display:grid;grid-template-columns:repeat(4,1fr);gap:9px;padding:15px 0}
.perfilFuncionarioResumo>div{padding:10px 11px;border:1px solid #e5ebf2;border-radius:10px;background:#fafcff;min-width:0}
.perfilFuncionarioResumo span,.perfilCampo span{display:block;color:#738096;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.25px}
.perfilFuncionarioResumo strong{display:block;margin-top:4px;color:#172033;font-size:11px;overflow-wrap:anywhere}
.perfilFuncionarioAbas{display:flex;gap:3px;overflow-x:auto;border-bottom:1px solid #dfe6ee;margin-bottom:14px}
.perfilFuncionarioAbas span{padding:9px 12px;color:#6a7689;font-size:10px;font-weight:800;white-space:nowrap;border-bottom:2px solid transparent}
.perfilFuncionarioAbas span.ativa{color:#087cf0;border-bottom-color:#087cf0}
.perfilFuncionarioGrid{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}
.perfilInfoCard{border:1px solid #e3e9f0;border-radius:12px;background:#fff;padding:13px}
.perfilInfoCabecalho{display:flex;align-items:center;gap:8px;padding-bottom:10px;margin-bottom:4px;border-bottom:1px solid #edf1f5;color:#172033;font-size:11px}
.perfilInfoCabecalho span{font-size:15px}
.perfilCampo{padding:8px 0;border-bottom:1px solid #f0f3f6}
.perfilCampo:last-child{border-bottom:0}
.perfilCampo strong{display:block;margin-top:3px;color:#253044;font-size:11px;overflow-wrap:anywhere}
.perfilMiniResumo{display:flex;align-items:baseline;gap:7px;padding:9px 0;border-bottom:1px solid #f0f3f6}
.perfilMiniResumo strong{font-size:18px;color:#087cf0}
.perfilMiniResumo span{font-size:10px;color:#66748a}
.perfilGerenciarLargo{width:100%;margin-top:11px;padding:9px;border:1px solid #cfe0f2;border-radius:9px;background:#f5f9fe;color:#0873d7;font-size:10px;font-weight:800;cursor:pointer}
.perfilFuncionarioRodape{display:flex;justify-content:flex-end;gap:8px;margin-top:16px;padding-top:13px;border-top:1px solid #e5ebf2}
@media(max-width:800px){
  .funcionariosResumo{align-items:flex-start;flex-direction:column}
  .funcionariosIndicadores{width:100%}
  .funcionarioIndicador{flex:1;min-width:0}
  .funcionariosTabela{display:none}
  .funcionariosTabelaWrap{overflow:visible;border:0;background:transparent}
  .funcionariosMobileList{display:grid;gap:9px}
  .funcionarioMobileCard{padding:12px;border:1px solid #e1e8f0;border-radius:13px;background:#fff;box-shadow:0 3px 12px rgba(15,23,42,.04)}
  .funcionarioMobilePrincipal{width:100%;display:flex;align-items:center;gap:10px;padding:0;border:0;background:transparent;text-align:left;cursor:pointer}
  .funcionarioMobileIdentidade{display:flex;flex-direction:column;min-width:0;flex:1}
  .funcionarioMobileIdentidade strong{font-size:13px;color:#172033;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .funcionarioMobileIdentidade small{margin-top:3px;color:#68758a;font-size:10px}
  .funcionarioMobileDados{display:grid;grid-template-columns:1.25fr 1fr .8fr;gap:7px;margin-top:11px}
  .funcionarioMobileDados>div{min-width:0;padding:8px;border-radius:9px;background:#f7f9fc;border:1px solid #e9eef4}
  .funcionarioMobileDados span{display:block;color:#7a8798;font-size:8px;font-weight:700;text-transform:uppercase}
  .funcionarioMobileDados strong{display:block;margin-top:3px;color:#253044;font-size:9px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
  .funcionarioMobileAcoes{display:flex;align-items:center;gap:6px;margin-top:10px}
  .funcionarioMobileAcoes .funcionarioVerBtn{flex:1}
  .funcionarioMobileAcoes .funcionarioEditarBtn{padding-left:10px;padding-right:10px}
  .perfilFuncionarioTopo{align-items:flex-start;flex-direction:column}
  .perfilFuncionarioAcoes{width:100%;justify-content:stretch}
  .funcionarioEditarPrincipal,.funcionarioGerenciarBtn{flex:1}
  .perfilFuncionarioResumo{grid-template-columns:repeat(2,1fr)}
  .perfilFuncionarioGrid{grid-template-columns:1fr}
  .perfilFuncionarioRodape{justify-content:stretch}
  .perfilFuncionarioRodape button{flex:1}
}
@media(max-width:480px){
  .funcionariosTitulo p{font-size:10px}
  .funcionariosIndicadores{gap:6px}
  .funcionarioIndicador{padding:8px 6px}
  .funcionarioIndicador strong{font-size:16px}
  .funcionarioMobileDados{grid-template-columns:1fr 1fr}
  .funcionarioMobileDados>div:last-child{grid-column:1/-1}
  .perfilFuncionarioAvatar{width:54px;height:54px;font-size:26px}
  .perfilFuncionarioNomeLinha h2{font-size:17px}
  .perfilFuncionarioResumo{gap:6px}
  .perfilFuncionarioResumo>div{padding:9px}
}
`;
