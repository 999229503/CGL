...
@media(max-width:700px){
  ...
  .mobileFriendlyTable .mobileActionButtons .materialCopyButton{
    background:#eaf8ff;
    border-color:#bfe4f5;
    color:#147fa8;
    font-weight:800;
...
const adicionarEtapa = () => {
  ...
  const copiarDadosMaterial = async (material: Material) => {
    const total = material.quantidade * material.valor;
    const texto = [
      `Material: ${material.nome}`,
      `Obra: ${material.obra || "-"}`,
      `Quantidade: ${material.quantidade} ${material.unidade}`,
      `Valor unitário: ${dinheiro(material.valor)}`,
      `Total: ${dinheiro(total)}`,
    ].join("\n");

    try {
      await navigator.clipboard.writeText(texto);
      alert("Dados do material copiados! Agora é só colar e enviar para o cliente.");
    } catch {
      const area = document.createElement("textarea");
      area.value = texto;
      area.style.position = "fixed";
      ...
      try {
        ...
        alert("Não foi possível copiar os dados do material.");
      }
  ...
  const criarPdf = (paginas: PdfPagina[]) => {
    ...
    for (const pagina of paginas) {
      ...
      const ids: number[] = [];
      for (const imagem of pagina.imagens) ids.push(proximoId++);
  ...
  const gerarPdfMateriais = async () => {
    ...
    try {
      const linhas: string[] = [];
      linhas.push("CGL - Gerenciamento de Obras");
      linhas.push("RELATÓRIO DE MATERIAIS");
      linhas.push(`Gerado em ${new Date().toLocaleString("pt-BR")}`);
      linhas.push("");
      linhas.push("OBRA | MATERIAL | QUANTIDADE | VALOR UNIT. | TOTAL");
      materiais.forEach((m) => {
        linhas.push(`${m.obra || "-"} | ${m.nome} | ${m.quantidade} ${m.unidade} | ${dinheiro(m.valor)} | ${dinheiro(m.quantidade * m.valor)}`);
      });
      const total = materiais.reduce((soma, m) => soma + m.quantidade * m.valor, 0);
      linhas.push("");
      linhas.push(`TOTAL GERAL: ${dinheiro(total)}`);

      const paginas: PdfPagina[] = [];
      let paginaLinhas: string[] = [];
      const criarPagina = (itens: string[]) => {
        let y = 800;
        let conteudo = "";
        itens.forEach((linha, i) => {
          const tamanho = i === 0 ? 18 : i === 1 ? 14 : i === 2 ? 8 : 9;
          const espaco = i === 0 ? 25 : i === 1 ? 22 : 15;
          conteudo += linhaPdf(linha, 42, y, tamanho, i < 2 || linha.startsWith("TOTAL"));
          y -= espaco;
        });
        paginas.push({ conteudo, imagens: [] });
      };
      linhas.forEach((linha) => {
        const partesLinha = quebrarTextoPdf(linha, 90);
        for (const parte of partesLinha) {
          if (paginaLinhas.length >= 48) { criarPagina(paginaLinhas); paginaLinhas = []; }
          paginaLinhas.push(parte);
        }
      });
      if (paginaLinhas.length || !paginas.length) criarPagina(paginaLinhas);
      const blob = criarPdf(paginas);
      const url = URL.createObjectURL(blob);
      setPdfGerado((anterior) => { if (anterior) URL.revokeObjectURL(anterior.url); return { url, nome: `CGL-Materiais-${hoje}.pdf`, tipo: "materiais" }; });
      const link = document.createElement("a"); link.href = url; link.download = `CGL-Materiais-${hoje}.pdf`; document.body.appendChild(link); link.click(); link.remove();
                  ...
                  <tr key={material.id}>
                    ...
                    <td>
                      <div className="mobileActionButtons">
                        <button
                          className="secondary materialCopyButton"
                          type="button"
                          onClick={() => copiarDadosMaterial(material)}
                        >
                          📋 Copiar
                        </button>
