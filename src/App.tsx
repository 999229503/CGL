...
@media(max-width:700px){.photoTools{grid-template-columns:1fr}.photoActions{width:100%}.photoActions button{flex:1}}`;
...
function App() {
  ...
  const salvarDadosLocalmente = (dados: CloudData) => {
    try {
      localStorage.setItem(CHAVE_DADOS, JSON.stringify(dados));
    } catch (erro) {
      console.warn("Não foi possível salvar no dispositivo:", erro);
    }
  };

  const salvarAlteracaoImediata = async (dados: CloudData) => {
    ...
    try {
      ...
      console.error("Não foi possível salvar a alteração online:", erro);
    } finally {
      setSincronizando(false);
    }
  };
  ...
  const mudarStatusObra = (id: number, status: Status) => {
    const novasObras = obras.map((item) =>
