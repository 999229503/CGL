...
function App() {
  ...
  const [gerandoPdf, setGerandoPdf] = useState<"diario" | "materiais" | null>(null);
...
const adicionarEtapa = () => {
  ...
  const criarPdf = (paginas: PdfPagina[]) => {
    ...
    paginas.forEach((pagina, indicePagina) => {
      ...
      const recursos = `<< /Font << /F1 ${fonteId} 0 R >>${xobjects ? ` /XObject << ${xobjects} >>` : ""} >>`;
  ...
  const linhaPdf = (texto: string, x: number, y: number, tamanho = 10, negrito = false) =>
    `BT /F1 ${tamanho} Tf ${negrito ? "1" : "0"} Tr ${x} ${y} Td (${escaparPdf(texto)}) Tj ET\n`;

  const gerarPdfMateriais = async () => {
    setGerandoPdf("materiais");
    try {
      const larguraPagina = 595;
      const margem = 36;
      const topo = 806;
      const rodape = 32;
      const materiaisOrdenados = materiais.slice();

      const desenharCabecalho = (y: number, paginaNumero: number) => {
        let c = "";
        c += linhaPdf("CGL - Gerenciamento de Obras", margem, y, 18, true);
        c += linhaPdf("RELATORIO DE MATERIAIS", margem, y - 24, 13, true);
        c += linhaPdf(
          `${materiaisOrdenados.length} ${materiaisOrdenados.length === 1 ? "material" : "materiais"}  |  Gerado em ${new Date().toLocaleDateString("pt-BR")}`,
          margem, y - 42, 8, false
        );
        c += `0.91 0.94 0.97 rg ${margem} ${y - 70} ${larguraPagina - margem * 2} 24 re f\n`;
        c += linhaPdf("OBRA", margem + 7, y - 61, 8, true);
        c += linhaPdf("MATERIAL", 178, y - 61, 8, true);
        c += linhaPdf("QTD.", 365, y - 61, 8, true);
        c += linhaPdf("VALOR UNIT.", 420, y - 61, 8, true);
        c += linhaPdf("TOTAL", 510, y - 61, 8, true);
        c += `0.78 0.84 0.89 RG 0.7 w ${margem} ${y - 70} ${larguraPagina - margem * 2} 0.1 re f\n`;
        if (paginaNumero > 1) {
          c += linhaPdf(`Continua - pagina ${paginaNumero}`, 455, y, 7, false);
        }
        return c;
      };

      const paginas: PdfPagina[] = [];
      let paginaConteudo = desenharCabecalho(topo, 1);
      let paginaNumero = 1;
      let y = topo - 82;

      const criarNovaPagina = () => {
        paginaConteudo += linhaPdf(`Pagina ${paginaNumero}`, margem, rodape, 7, false);
        paginas.push({ conteudo: paginaConteudo, imagens: [] });
        paginaNumero += 1;
        paginaConteudo = desenharCabecalho(topo, paginaNumero);
        y = topo - 82;
      };

      materiaisOrdenados.forEach((m, indice) => {
        const obra = quebrarTextoPdf(m.obra || "-", 25);
        const nome = quebrarTextoPdf(m.nome || "-", 27);
        const linhasDaLinha = Math.max(obra.length, nome.length, 1);
        const alturaLinha = Math.max(22, linhasDaLinha * 11 + 9);

        if (y - alturaLinha < rodape + 34) criarNovaPagina();

        if (indice % 2 === 1) {
          paginaConteudo += `0.98 0.99 1 rg ${margem} ${y - alturaLinha + 2} ${larguraPagina - margem * 2} ${alturaLinha} re f\n`;
        }

        for (let i = 0; i < linhasDaLinha; i++) {
          const yy = y - 11 - i * 11;
          paginaConteudo += linhaPdf(obra[i] || "", margem + 7, yy, 8.5, false);
          paginaConteudo += linhaPdf(nome[i] || "", 178, yy, 8.5, i === 0);
        }

        paginaConteudo += linhaPdf(`${m.quantidade} ${m.unidade}`, 365, y - 11, 8.5, false);
        paginaConteudo += linhaPdf(dinheiro(m.valor), 420, y - 11, 8.5, false);
        paginaConteudo += linhaPdf(dinheiro(m.quantidade * m.valor), 510, y - 11, 8, true);
        paginaConteudo += `0.86 0.89 0.92 RG 0.5 w ${margem} ${y - alturaLinha + 1} ${larguraPagina - margem * 2} 0.1 re f\n`;

        y -= alturaLinha;
      });

      const total = materiaisOrdenados.reduce((soma, m) => soma + m.quantidade * m.valor, 0);
      if (y - 55 < rodape) criarNovaPagina();

      paginaConteudo += `0.91 0.95 0.98 rg ${margem} ${y - 42} ${larguraPagina - margem * 2} 38 re f\n`;
      paginaConteudo += linhaPdf("TOTAL GERAL", margem + 10, y - 19, 11, true);
      paginaConteudo += linhaPdf(dinheiro(total), 455, y - 19, 12, true);
      paginaConteudo += linhaPdf(`Total de itens: ${materiaisOrdenados.reduce((s, m) => s + Number(m.quantidade || 0), 0)}`, margem + 10, y - 34, 7, false);
      paginaConteudo += linhaPdf(`Pagina ${paginaNumero}`, margem, rodape, 7, false);
      paginas.push({ conteudo: paginaConteudo, imagens: [] });
      ...
      setPdfGerado((anterior) => {
        ...
        return { url, nome: `CGL-Materiais-${hoje}.pdf`, tipo: "materiais" };
      });
      const link = document.createElement("a");
      link.href = url;
      link.download = `CGL-Materiais-${hoje}.pdf`;
  ...
  const gerarPdfDiario = async () => {
    setGerandoPdf("diario");
    try {
      const paginas: PdfPagina[] = [];
      const registros = diarioObra.slice().sort((a, b) => String(b.data).localeCompare(String(a.data)));

      const dataFormatada = (data: string) =>
        data ? `${data.slice(8, 10)}/${data.slice(5, 7)}/${data.slice(0, 4)}` : "-";

      const adicionarRodape = (conteudo: string, pagina: number) =>
        conteudo + linhaPdf(`CGL • Diário de Obra • Página ${pagina}`, 36, 24, 7, false);

      if (!registros.length) {
        paginas.push({
          conteudo:
            linhaPdf("CGL - Gerenciamento de Obras", 36, 806, 18, true) +
            linhaPdf("DIÁRIO DE OBRA", 36, 780, 14, true) +
            linhaPdf("Nenhum registro cadastrado.", 36, 742, 10),
          imagens: [],
        });
      } else {
        for (const registro of registros) {
          ...
          const linhasDescricao = quebrarTextoPdf(registro.descricao || "-", 92);
          const linhasObservacao = registro.observacao ? quebrarTextoPdf(registro.observacao, 92) : [];

          // Cabeçalho e texto ocupam apenas o espaço necessário.
          let conteudo = "";
          let y = 806;
          conteudo += linhaPdf("CGL - Gerenciamento de Obras", 36, y, 18, true);
          y -= 25;
          conteudo += linhaPdf("DIÁRIO DE OBRA", 36, y, 14, true);
          y -= 25;
          conteudo += linhaPdf(`Data: ${dataFormatada(registro.data)}`, 36, y, 9, true);
          conteudo += linhaPdf(`Obra: ${registro.obra || "-"}`, 210, y, 9, true);
          y -= 18;
          conteudo += linhaPdf(`Título: ${registro.titulo || "-"}`, 36, y, 9, true);
          y -= 18;

          if (registro.etapa) {
            conteudo += linhaPdf(`Etapa / Serviço: ${registro.etapa}`, 36, y, 9, false);
            y -= 18;
          }

          conteudo += `0.93 0.96 0.98 rg 36 ${y - 5} 523 18 re f\n`;
          conteudo += linhaPdf("O QUE FOI FEITO", 44, y, 9, true);
          y -= 20;

          for (const linha of linhasDescricao) {
            if (y < 70) break;
            conteudo += linhaPdf(linha, 44, y, 9, false);
            y -= 12;
          }

          if (registro.observacao && y > 90) {
            y -= 5;
            conteudo += `0.93 0.96 0.98 rg 36 ${y - 5} 523 18 re f\n`;
            conteudo += linhaPdf("OBSERVAÇÕES", 44, y, 9, true);
            y -= 20;
          ...
          // Reserva somente o espaço realmente necessário para as fotos.
          if (imagens.length) {
            y -= 8;
            const colunas = 2;
            const colGap = 14;
            const larguraFoto = 250;
            const alturaFotoMax = 145;
            const linhaFotoAltura = alturaFotoMax + 30;
            const linhasFotos = Math.ceil(imagens.length / colunas);
            const espacoFotos = linhasFotos * linhaFotoAltura + 26;

            // Se as fotos não couberem junto do texto, fecha esta página e continua.
            if (y - espacoFotos < 45) {
              paginas.push({ conteudo: adicionarRodape(conteudo, paginas.length + 1), imagens: [] });
              conteudo = "";
              y = 806;
              conteudo += linhaPdf("DIÁRIO DE OBRA", 36, y, 14, true);
              y -= 22;
              conteudo += linhaPdf(`${registro.obra || "-"} • ${registro.titulo || "-"}`, 36, y, 9, true);
              y -= 20;
            }

            conteudo += `0.93 0.96 0.98 rg 36 ${y - 5} 523 18 re f\n`;
            conteudo += linhaPdf(`FOTOS DO DIA • ${imagens.length}`, 44, y, 9, true);
            y -= 22;
            ...
            imagens.forEach((imagem, i) => {
              ...
              const escala = Math.min(
                larguraFoto / imagem.largura,
                alturaFotoMax / imagem.altura
              );
              const w = imagem.largura * escala;
              const h = imagem.altura * escala;
              const xBase = 36 + coluna * (larguraFoto + colGap);
              const x = xBase + (larguraFoto - w) / 2;
              const yBase = y - linha * linhaFotoAltura - h;

              conteudo += `q ${w.toFixed(2)} 0 0 ${h.toFixed(2)} ${x.toFixed(2)} ${yBase.toFixed(2)} cm /Im${i + 1} Do Q\n`;
              if (imagem.nome) {
                conteudo += linhaPdf(
                  quebrarTextoPdf(imagem.nome, 38)[0],
                  xBase,
                  y - linha * linhaFotoAltura - alturaFotoMax - 13,
                  7,
                  false
                );
              }
            ...
            paginas.push({
              conteudo: adicionarRodape(conteudo, paginas.length + 1),
              imagens,
            });
          } else {
            paginas.push({
      ...
      setPdfGerado((anterior) => {
        if (anterior) URL.revokeObjectURL(anterior.url);
        return { url, nome: `CGL-Diario-de-Obra-${hoje}.pdf`, tipo: "diario" };
      });
      const link = document.createElement("a");
      link.href = url;
      link.download = `CGL-Diario-de-Obra-${hoje}.pdf`;
      document.body.appendChild(link);
      link.click();
      link.remove();
    } finally {
      ...
      <section className="panel">
        <div className="panelHeader">
          ...
          <div className="pdfAcoes">
            <button type="button" className="secondary pdfBtn" disabled={gerandoPdf === "materiais"} onClick={() => void gerarPdfMateriais()}>📄 {gerandoPdf === "materiais" ? "Gerando..." : "PDF"}</button>
