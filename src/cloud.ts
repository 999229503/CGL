...
const STORAGE_BUCKET = "cgl-fotos";
...
async function storageRequest(path: string, options: RequestInit = {}, token?: string) {
  ensureConfigured();
  const headers = new Headers(options.headers);
  headers.set("apikey", SUPABASE_KEY);
  ...
  if (!response.ok) {
    const message = typeof body === "object" && body !== null
      ? String((body as Record<string, unknown>).message || (body as Record<string, unknown>).error || (body as Record<string, unknown>).statusCode || "Erro no armazenamento")
      : String(body || `Erro HTTP ${response.status}`);
    throw new Error(message);
  }
  return body;
...
export async function uploadImageDataUrl(session: AuthSession, dataUrl: string, storagePath: string): Promise<string> {
  if (!dataUrl.startsWith("data:image/")) return dataUrl;
  const blob = await fetch(dataUrl).then((response) => response.blob());
  await storageRequest(`/storage/v1/object/${STORAGE_BUCKET}/${encodeStoragePath(storagePath)}`, {
    method: "POST",
    headers: {
...
export async function getSignedImageUrl(session: AuthSession, storagePath: string, expiresIn = 604800): Promise<string> {
  const result = await storageRequest(`/storage/v1/object/sign/${STORAGE_BUCKET}/${encodeStoragePath(storagePath)}`, {
    method: "POST",
    body: JSON.stringify({ expiresIn }),
  }, session.access_token) as { signedURL?: string };
  const signed = String(result?.signedURL || "");
  if (!signed) throw new Error("O Supabase não retornou a URL assinada da imagem.");
  return signed.startsWith("http") ? signed : `${SUPABASE_URL}${signed.startsWith("/") ? "" : "/"}${signed}`;
}

export async function deleteImage(session: AuthSession, storagePath: string): Promise<void> {
  await storageRequest(`/storage/v1/object/${STORAGE_BUCKET}/${encodeStoragePath(storagePath)}`, {
    method: "DELETE",
  }, session.access_token);
}
...
export async function saveCloudData(session: AuthSession, data: CloudData): Promise<string> {
  ...
  await request(
    "/rest/v1/obracontrol_data?on_conflict=user_id",
    {
      method: "POST",
      headers: {
