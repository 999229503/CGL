...
export async function saveCloudData(session: AuthSession, data: CloudData): Promise<string> {
  ...
  // Primeiro tenta atualizar a linha existente do usuário.
  // Isso evita depender de uma constraint UNIQUE em user_id para o upsert.
  const updated = await request(
    `/rest/v1/obracontrol_data?user_id=eq.${encodeURIComponent(session.user.id)}`,
    {
      method: "PATCH",
  ...
  // Se ainda não existir uma linha para este usuário, cria a primeira.
