...
export async function getCloudDataUpdatedAt(session: AuthSession): Promise<string | null> {
  const rows = await request(
    `/rest/v1/obracontrol_data?select=updated_at&user_id=eq.${encodeURIComponent(session.user.id)}&limit=1`,
    { method: "GET" },
...
export async function saveCloudData(session: AuthSession, data: CloudData): Promise<string> {
  ...
  // A tabela usa user_id como chave primária. Então o upsert é a forma mais
  // segura de salvar: se a linha existir, atualiza; se não existir, cria.
  // Isso evita a janela de corrida PATCH -> POST e garante um único registro
  // por usuário.
  await request(
